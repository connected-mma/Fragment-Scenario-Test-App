...
@Unscoped
class PelotonSavedStateViewModelFactory @Inject constructor(
            ...
            override fun <T : ViewModel> create(
                ...
                val viewModel =
                    createAssistedInjectViewModel(modelClass, handle)
                        ?: createInjectViewModel(modelClass, defaultArgs)
     ...
     */
    private fun <T : ViewModel?> createInjectViewModel(modelClass: Class<T>, defaultArgs: Bundle? = null): ViewModel? {
        val creator = viewModelProviders[modelClass]
            ?: viewModelProviders.asIterable()
                .firstOrNull { modelClass.isAssignableFrom(it.key) }?.value
            ?: return null

        return creator.get().apply {
            // Trigger the parameter loading from the given bundle if this is a ViewModelWithParameter
            (this as? ViewModelWithParameter<*>)?.loadParameter(defaultArgs)
